...
const SearchFlight = () => {
  ...
  const handleOnSubmit = async (e) => {
    try {
      ...
      //router.push(`/searchFlight?origin=${searchObj.originLocationCode}&destination=${searchObj.destinationLocationCode}&date=${searchObj.departureDate.toISOString().substring(0, 10)}${!oneWay ? `&returnDate=${searchObj.returnDate.toISOString().substring(0, 10)}` : ''}&adults=${searchObj.adults}&token=${accessToken}&oneWay=${oneWay.toString()}`);
        router.replace(`/searchFlight?origin=${searchObj.e}&destination=${searchObj.destinationLocationCode}&date=${searchObj.departureDate.toISOString().substring(0, 10)}${!oneWay ? `&returnDate=${searchObj.returnDate.toISOString().substring(0, 10)}` : ''}&adults=${searchObj.adults}&token=${accessToken}&oneWay=${oneWay.toString()}`)
  ...
  const handleEarliest = async () => {
    ...
    try {
      const response = await fetch("https://api.amadeus.com/v2/shopping/flight-offers", {
        ...
        body: oneWay ? JSON.stringify(queryEarliest) : JSON.stringify(queryEarliestRound)
      });
      const json = await response.json();
      console.log(json, "JSON");
      ...
      if (oneWay)
        setFlightList(newFlightListEarliest);
      else {
        // const twoWay = [...newFlightList, ...newFlightList1];
        // setFlightList(twoWay);
        const twoWay = [...(newFlightListEarliest || []), ...(newFlightListEarliest1 || [])];
        setFlightList(twoWay);
        console.log(twoWay, "FlightList in searchflight")
  ...
  //       },
  //       "sortCriteria": {
  //         "sortOrder": "QUICKEST" // Changed to sort by quickest travel time
  //       }
  //     }
  //   };

  //   let queryQuickesttRound = {
  //     "currencyCode": "USD",
  //     "originDestinations": [
  //       {
  //         "id": "1",
  ...
  //         "Authorization": `Bearer ${searchParam.get("token")}`
  //       },
  //       body: oneWay ? JSON.stringify(queryQuickest) : JSON.stringify(queryQuickesttRound)
  //     });
  //     const json = await response.json();
  //     console.log(json, "JSON");
  //     const newFlightListQuickest = json.data.map(a => {
  //       a.stops = a.itineraries[0].segments.length - 1;
  //       a.itineraries.forEach(b => {
  //         b.segments.forEach(segment => {
  //           segment.airline = airlines[segment.carrierCode];
  ...
  //     let newFlightListQuickest1;
  //     if (!oneWay) {
  //       newFlightListQuickest1 = json.data.map(a => {
  //         a.stops = a.itineraries[1].segments.length - 1;
  //         a.itineraries.forEach(b => {
  //           b.segments.forEach(segment => {
  //             segment.airline = airlines[segment.carrierCode];
  ...
  //     if (oneWay)
  //       setFlightList(newFlightListQuickest);
  //     else {
  //       // const twoWay = [...newFlightList, ...newFlightList1];
  //       // setFlightList(twoWay);
  //       const twoWay = [...(newFlightListQuickest || []), ...(newFlightListQuickest || [])];
  //       setFlightList(twoWay);
  //       console.log(twoWay, "FlightList in searchflight")
    ...
    return [...FlightList]
      .sort((a, b) => {
        const durationA = a.itineraries.reduce((total, itinerary) => {
          return total + calculateTotalDuration(itinerary);
        }, 0);
    ...
    <div className="">
      ...
      <div className="container-fluid">
        <div className="landing-widget">
          ...
          <ModifyForm oneWay={oneWay.toString()} setFlightDetails={setFlightDetails} setFlightList={setFlightList} FlightList={FlightList}/>
        </div>
      </div>
    </div>
    {/* main Listing */}
        ...
        <div className="right-listing right-bar">
          ...
          <div className="shortby  sort-area" id="dvChpfastprice">
            <ul>
              ...
              <li className="tipsy-fastest sort-traveltime" onClick={handleQuickest} >
                <input
                  type="hidden"
              ...
              <li className="tipsy-earliest sort-earliest" onClick={handleEarliest}>
                <input
                  type="hidden"
                  id="hdnsortearliest"
          ...
          {!earliest && FlightList && FlightList.map(a => {

            return <SearchFlightCard setSelectedFlight={setSelectedFlight} setFlightDetailVisible={setFlightDetailVisible} flight={a} oneWay={oneWay.toString()} token={searchParam.get("token")} />
          ...
          {quickest && FlightList && FlightList.map(a => {
